...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 */
	/**
	 * BeanFactory、FactoryBean、ObjectFactory是不同的
	 * 在getBean方法中，getObjectForBeanInstance是个高频率使用的方法，无论是从缓存中
	 * 获得bean还是根据不同的scope策略加载bean。总之，我们得到bean的实例后要做的第一
	 * 步就是调用这个方法来检测一下正确性，其实就是用于检测当前bean是否是FactoryBean
	 * 类型的bean，如果是，那么需要调用该bean对应的FactoryBean实例中的getObJect()作为返回值
	 * 无论是从缓存中获取到的bean还是通过不同的scope策略加载的都只是最原始的bean
	 * 状态，并不一定是我们最终想要的bean举个例子，假如我们需要对工厂bean进行处理，那么这里得到的其实是工厂bean的初始状态，
	 * 但是我们真正需要的是工厂bean反中定义的factory-method方法中返回的bean，而getObJectForBeanlnstance方法就是完成这个工作的
	 * @param beanInstance
	 * @param name
	 * @param beanName
	 * @param mbd
	...
	protected Object getObjectForBeanInstance(
		...
		// 如果指定的 name 是工厂相关(以&为前缀)且beanInstance又不是FactoryBean类型则检验不通过
		...
		// 现在我们有了bean的实例，这个实例可能会是正常的 bean 或者是 FactoryBean
		// 如果是FactoryBean我们使用它创建实例，但是如果用户想要直接获取工厂实例而不是工厂的 getObject 方法返回的实例，那么传入的name应该加上 & 前缀
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		...
		if (object == null) {
			...
			// containsBeanDefinition 检测 beanDefinitionMap(xml所映射的bean的内部表现形式) 中也就是在所有已加载的类中检测是否定义 beanName
			if (mbd == null && containsBeanDefinition(beanName)) {
				// 将用于存储XML配置文件的GernericBeanDefinition转换为RootBeanDefinition，如果BeanName是子bean的话会同时合并父类的相关属性
				mbd = getMergedLocalBeanDefinition(beanName);
			}
			// 是否是用户定义的而不是应用程序本身定义的
			boolean synthetic = (mbd != null && mbd.isSynthetic());
			// 从 FactoryBean 中获取正确的bean的核心代码
